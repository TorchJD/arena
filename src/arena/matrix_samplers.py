...
class MatrixSampler(ABC):
    ...
    def __str__(self) -> str:
        return f"{self.__class__.__name__.replace('MatrixSampler', '')}" f"({self.m}x{self.n}r{self.rank}:{str(self.dtype)[6:]})"
...
def _sample_orthonormal_matrix(dim: int, dtype: torch.dtype, rng: torch.Generator | None = None) -> Tensor:
